...
void check_winners(vector<board> *boards, vector<board> *winners) {
...
int mark_boards(vector<int> *picks, vector<board> *boards, vector<board> *winners) {
...
int parse_file(ifstream *file) {
  ...
  int winning_pick = mark_boards(&picks, &boards, &winners);
  return score_winner(winning_pick, &winners.at(0));
}
